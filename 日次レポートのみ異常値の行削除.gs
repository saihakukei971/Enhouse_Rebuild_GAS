function deleteRowsForNichiReport() {
    Logger.log("[INFO] スクリプト開始");

    const sheetName = "日次レポート"; // 対象シート
    const numHeaderRow = 2; // ヘッダー行を除外

    // **全シートを取得して存在を確認**
    const sheets = SpreadsheetApp.getActiveSpreadsheet().getSheets();
    sheets.forEach(s => Logger.log("[INFO] 検出シート: " + s.getName()));
    ...
    Logger.log("[DEBUG] 取得データの先頭: " + JSON.stringify(range[0]));
    Logger.log("[DEBUG] 取得データの最終行: " + JSON.stringify(range[range.length - 1]));
